...
class Main(QtWidgets.QMainWindow, home.Ui_MainWindow):
    ...
    def ImageUpdateSlot(self, Image, state):
        if(state == "None"):
...
class rfidThread(QThread):
    ...
    def saveToCsv(self,Info):
        ...
        with open('Driver.csv', 'a') as csvfile:
            fieldNames = ['NAME', 'CONTACT NUMBER', 'PLATE NUMBER', 'CODING DAY' , 'TIME', 'DATE']
            writer = csv.DictWriter(csvfile, fieldnames=fieldNames)
            writer.writerow({'NAME': Info[0],'CONTACT NUMBER': Info[1], 'PLATE NUMBER': Info[2], 'CODING DAY': Info[3], 'TIME': self.current_time, 'DATE': self.today})
...
class qrThread(QThread):
    ...
    def run(self):
        ...
        while self.ThreadActive:
            ...
            if ret:
                ...
                self.ImageUpdate.emit(Pic,"None")     #Produces a signal in which the Main class will receive and Image and an Int Value example: (Pic,0)
                if bbox is not None:
                    ...
                    if data:
                        ...
                        self.saveToCSV(dataList)        #save to csv
                        self.ImageUpdate.emit(Pic,str(dataList[0])) #Produces a signal in which the Main class will receive and Image and an Int Value example: (Pic,1)
                        Capture.release()
                        QThread.sleep(3)
                        Capture = cv2.VideoCapture(0)
    ...
    def saveToCSV(self,Info):
        ...
        with open('Passenger.csv', 'a') as csvfile:
            fieldNames = ['NAME', 'AGE', 'ADDRESS' , 'CONTACT NUMBER' , 'TIME', 'DATE']
            writer = csv.DictWriter(csvfile, fieldnames=fieldNames)
            writer.writerow({'NAME': Info[0], 'AGE': Info[1], 'ADDRESS': Info[2], 'CONTACT NUMBER': Info[3], 'TIME': self.current_time, 'DATE': self.today})
        #TODO qrThread.led.toggle()
